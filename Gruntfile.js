...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    connect: {
      ...
      proxies: [{
        context: '/api',
        host: '<%= php.dist.options.hostname %>',
        port: '<%= php.dist.options.port %>'
      }],
      livereload: {
        options: {
          open: false,
    ...
    clean: {
      ...
      php: {
        src: [
          '<%= yeoman.dist %>/api/stones/*',
          '<%= yeoman.dist %>/api/db.sqlite3',
          '<%= yeoman.dist %>/api/phpliteadmin.php',
          '<%= yeoman.dist %>/api/mgmt/users.db'
        ]
      }
    },
